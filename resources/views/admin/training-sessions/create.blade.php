<x-app-layout>
  ...
  <x-admin.page-content>
    <x-admin.card>
      <form method="POST" action="{{ route('admin.training-sessions.store') }}" class="space-y-4 max-w-xl">
        @csrf
        ...
        <div><label class="block text-sm font-medium">{{ __('Date') }}</label><input type="date" name="date" class="mt-1 w-full rounded-lg border-pb-border" required></div>
        <div class="grid grid-cols-2 gap-4">
          <div><label class="block text-sm font-medium">{{ __('Start') }}</label><input type="time" name="start_time" class="mt-1 w-full rounded-lg border-pb-border"></div>
          <div><label class="block text-sm font-medium">{{ __('End') }}</label><input type="time" name="end_time" class="mt-1 w-full rounded-lg border-pb-border"></div>
        </div>
